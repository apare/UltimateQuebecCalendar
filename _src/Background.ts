...
namespace Background {
  ...
  function onMessage(
    request: { action: string; calandarId: string; event?: GoogleApi.Event },
    tabId: number,
    sendResponse: Function
  ) {
    if (request.action == "createEvent" || request.action == "deleteEvent") {
      GoogleApi[
        request.action == "createEvent" ? "createEvent" : "deleteEvent"
      ](request.calandarId, request.event)
        .then((event: GoogleApi.Event) => {
          chrome.tabs.executeScript(tabId, {
            code:
              "UltimateQuebecCalendar.updateEvent(" +
